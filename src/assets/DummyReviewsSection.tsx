            ...
            <div className="flex-center flex-col md:min-w-[160px] lg:w-1/3">
              ...
              <div className="flex items-center gap-0.5 mt-2 mb-1">
                {[1, 2, 3, 4, 5].map((s) => (
                  <StarIcon
                    key={s}
                    className={`size-4 ${s <= Math.round(product.rating) ? "text-red-600 star-fill-orange" : "text-app-border"}`}
                  />
                ))}
              </div>
              ...
              {breakdown.map((count, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2.5 bg-app-border rounded-full overflow-hidden">
                    <div
                      className="h-full star-fill-orange rounded-full transition-all duration-500"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-0.5 mb-2">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <StarIcon
                        key={s}
                        className={`size-3.5 ${s <= review.rating ? "text-red-600 star-fill-orange" : "text-app-border"}`}
                      />
                    ))}
                  </div>
